...
import numpy as np
# import torch # <-- Removed, not used in this version
import keras
...
class TensorParallelKeras(keras.Model):
    ...
    def __init__(self, model, world_size=None, device_ids=None, distributed_backend="auto", **kwargs):
        ...
        if keras.backend.backend() == 'jax' or distributed_backend == 'jax':
            try:
                ...
                print(f"🔍 Real JAX backend detected: {len(accel_devices)} devices available")
                print(f"🔍 Device types: {[str(d) for d in accel_devices]}")
                
                if len(accel_devices) >= world_size:
                    print(f"✅ JAX has {len(accel_devices)} devices, using REAL tensor parallelism on {world_size} devices")
                    # Use actual JAX device objects
                    device_ids = accel_devices[:world_size]
                    print(f"🔍 Using JAX devices: {device_ids}")
                else:
                    print(f"⚠️  JAX has {len(accel_devices)} devices but world_size={world_size}")
        ...
        if keras.backend.backend() == 'jax':
            try:
                import jax
                logger.info(f"✅ JAX backend confirmed for real parameter sharding")
            except Exception as e:
    ...
    def call(self, inputs, training=None, **kwargs):
        ...
        logger.debug(f"Combining shard outputs with sharding type: {sharding_type}")
    ...
    def compile(self, optimizer=None, loss=None, metrics=None, **kwargs):
        ...
        if len(self.model_shards) > 1 and optimizer is not None:
            ...
            logger.info("Compiled TensorParallelKeras model with coordinated optimizer.")
            ...
            try:
                ...
                logger.info(f"Compiled all {len(self.model_shards)} individual shards.")
            except Exception as e:
    ...
    def _get_learning_rate(self):
        ...
        try:
            if hasattr(self, 'coordinated_optimizer') and self.coordinated_optimizer:
